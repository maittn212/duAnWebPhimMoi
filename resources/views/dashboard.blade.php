                    ...
                    <ul class="list-group">
                        @foreach ($new_movies as $movie)
                            <li class="list-group-item recent-movie-item">
                                ...
                                @else
                                    <img src="{{ Storage::url($movie->image) }}" alt="{{ $movie->title }}"
                                        title="{{ $movie->title }}" width="60px" height="90px">
                                @endif
                                <div class="recent-movie-info">
                                    <strong>{{ $movie->title }}</strong><br>
                                    <small class="text-muted">{{ number_format($movie->count_views) }} lượt quan
                                        tâm</small><br>
                                    <small class="text-muted">Cập nhật ngày:
                                        {{ $movie->updated_at->format('d/m/Y') }}</small>
